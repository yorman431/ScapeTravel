...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" class="marco">
  ...
  <tr {if $cont eq '0'} class='listado_a'
    ...
    <td class="adminContenido">{$listado[i].ruta}</td>
    <td class="adminContenido">{$listado[i].costo}</td>
    <td class="adminContenido">{$listado[i].costo_nino}</td>
    <td class="adminContenido">{$listado[i].condiciones}</td>
    ...
    <td width="5%" align="center"><a title="Eliminar" onclick="javascript: return confirmar('&iquest;Seguro desea eliminar este registro?')" href="eliminar.php?id={$listado[i].id}"><img src="/imagenes/delete.png" width="25" height="25" border="0" /></a></td>
  </tr>
        {/section}
        {else}
        {$mensaje}
      ...
      {if $mensaje eq ""}
      {$paginate.first}-{$paginate.last} de {$paginate.total} Contenidos.

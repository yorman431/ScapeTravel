...
class Contenido extends Link{
	...
	function listar_contenido_imagen($tipo){
		...
		$sql="SELECT *, id_con AS directorio_image, id_con AS nombre_image FROM contenido WHERE $restriccion GROUP BY id_con ORDER BY fecha_con DESC, enlace_con, nombre_con";
		$consulta=mysql_query($sql) or die(mysql_error());
		while ($resultado = mysql_fetch_array($consulta)){
	...
	/*Metodo para editar un usuario seleccionado */	
		...
		if (isset($_POST['envio']) && $_POST['envio']=="Guardar"){
			...
			$sql="INSERT INTO contenido VALUES ('', '$this->nombre', '$this->enlace', '$this->subenlace', '$this->fecha', '$this->contenido')";
			$consulta=mysql_query($sql) or die(mysql_error());
	...
	/*Metodo para editar un usuario seleccionado */		
		...
		if (isset($_POST['envio']) && $_POST['envio']=="Guardar" && isset($_GET['id']) && $_GET['id']!=""){
			...
			$this->asignar_valores();
			$this->fecha=$this->convertir_fecha($this->fecha);
			$sql="UPDATE contenido SET nombre_con='$this->nombre', enlace_con='$this->enlace', subenlace_con='$this->subenlace', fecha_con='$this->fecha',  contenido_con='$this->contenido'  WHERE id_con='$id'";
			$consulta=mysql_query($sql) or die(mysql_error());
